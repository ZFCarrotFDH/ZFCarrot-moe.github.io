...
57. 粗糙脉孢菌 *cys-1* 基因有 1、2 和 3 三个突变，突变体相互杂交后可得到野生型后代，其中 1 和 2 杂交后出现野生型后代的频率为 $0.6 * 10^{-4}$ ，1 和 3 杂交后为 $0.7 * 10^{-4}$ ，2 和 3 杂交后为 $1.0 * 10^{-5}$ 。判断下列论述的正误：
    - A．据此推测，突变 1 位于中间； 
    - B．据此推测，突变 2 位于中间；
    - C．据此推测，突变 3 位于中间； 
...
62. 研究证明，DMD 是 X-染色体连锁的隐性遗传病。表中是对该地 1940~46 年出生的 138403 名男性中 DMD 患者的统计结果，发现群体中 d 基因的频率基本维持稳定。已知人类基因组的每个碱基在每一次代际传递时的突变率约为 $1.1 * 10^{-8}$。根据这些信息，判断下列相关论述的正误：
    - A. 1940~46 年间该群体中基因型频率基本不变；
    - B. 每一次代际传递，*D* 突变为 *d* 的概率约为 $1.3 * 10^{-4}$；
